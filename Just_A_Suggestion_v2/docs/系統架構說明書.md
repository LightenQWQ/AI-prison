...
### 第一層：藝術筆觸約定 (Strict Manga/Sketch Constraint)
在 `STYLE_CONSTRAINTS` 中硬性編碼：
> `"STRICTLY BLACK AND WHITE MANGA STYLE. Noir pencil sketch, heavy cross-hatching, high contrast ink, grainy halftones. NO PHOTOGRAPHY."`
這會強制 AI 放棄寫實攝影風格，改用強烈的排線與墨水筆觸。

### 第二層：HAMP 藝術避險協定 (Heuristic Art Metaphor Protocol)
在生成 `image_prompt` 前，強制將「光影、反射」等詞彙補強為 `(rendered in stark ink lines and rough pencil hatching, no realistic shaders)`。確保畫面不僅安全，且呈現出純粹的紙本藝術感。
